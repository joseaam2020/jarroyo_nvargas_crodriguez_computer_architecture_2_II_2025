...
SnoopModule::SnoopResponse SnoopModule::handleBusRead(int address) {
  ...
  case MESIState::MODIFIED:
    std::cout << "  [PE" << pe_id
              << " Snoop] BusRead hit in MODIFIED -> Flush to memory & "
  ...
  case MESIState::EXCLUSIVE:
    std::cout << "  [PE" << pe_id
              << " Snoop] BusRead hit in EXCLUSIVE -> transition to SHARED"
              << std::endl;
    response.shared = true;
    cache->setState(address, MESIState::SHARED);
    break;
  ...
  case MESIState::SHARED:
    std::cout << "  [PE" << pe_id
              << " Snoop] BusRead hit in SHARED -> remain SHARED" << std::endl;
    response.shared = true;
    break;
...
SnoopModule::SnoopResponse SnoopModule::handleBusReadX(int address) {
  ...
  case MESIState::MODIFIED:
    std::cout
        << "  [PE" << pe_id
        << " Snoop] BusReadX hit in MODIFIED -> Flush to memory & INVALIDATE"
        << std::endl;
    response.modified = true;
  ...
  case MESIState::EXCLUSIVE:
    std::cout << "  [PE" << pe_id
              << " Snoop] BusReadX hit in EXCLUSIVE -> INVALIDATE" << std::endl;
    cache->invalidateLine(address);
    break;

  case MESIState::SHARED:
    std::cout << "  [PE" << pe_id
              << " Snoop] BusReadX hit in SHARED -> INVALIDATE" << std::endl;
    cache->invalidateLine(address);
    break;
...
void SnoopModule::handleBusInvalidate(int address) {
  if (cache->hasLine(address)) {
    std::cout << "  [PE" << pe_id
              << " Snoop] Invalidate signal -> INVALIDATE line" << std::endl;
    cache->invalidateLine(address);
  }
}

void SnoopModule::processLocalRead(int address, bool shared_signal,
  ...
  MESIState new_state =
      shared_signal ? MESIState::SHARED : MESIState::EXCLUSIVE;
...
void SnoopModule::processLocalWrite(int address, int data) {
  ...
  case MESIState::MODIFIED:
    ...
    std::cout << "  [PE" << pe_id
              << " Local] Write hit in MODIFIED -> Update data" << std::endl;
    break;
  ...
  case MESIState::EXCLUSIVE:
    ...
    std::cout << "  [PE" << pe_id
              << " Local] Write hit in EXCLUSIVE -> MODIFIED" << std::endl;
    break;
  ...
  case MESIState::SHARED:
    ...
    cache->setState(address, MESIState::MODIFIED);
    std::cout << "  [PE" << pe_id
              << " Local] Write hit in SHARED -> Invalidate others & MODIFIED"
              << std::endl;
    break;

  case MESIState::INVALID:
    cache->insertLine(address, data, MESIState::MODIFIED);
    std::cout << "  [PE" << pe_id << " Local] Write miss -> Load & MODIFIED"
              << std::endl;
    break;
  }
}
