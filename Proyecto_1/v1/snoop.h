...
private:
    ...
    void transitionOnRead(int address, bool other_caches_have); // Metodo para decidir entre los estado S y E en caso de lectura
    void transitionOnWrite(int address); // Realiza cambio al estado M
    
    // Calcula los campos de dirección igual que Cache, para poder indicar la línea
...
public:
    ...
    // Cambios locales
    // Read miss local
    void processLocalRead(int address, bool shared_signal, double* data_from_source);
 
    // Escritura local (hit o miss)
    void processLocalWrite(int address, double value);
    

    // Operaciones con el Interconnect
    // Maneja un Read Miss (solicita el dato al interconnect, lo recibe y actualiza caché)
    bool handleReadMiss(int address);

    // Maneja un wirte, hit o miss, (si es miss, trae el dato, invalida las copias y actiliza caché)
    bool handleWrite(int address, double value);
